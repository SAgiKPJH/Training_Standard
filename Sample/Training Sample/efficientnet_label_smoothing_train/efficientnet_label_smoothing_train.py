...
def without_access_token(d:dict):
    from copy import deepcopy
    copied = deepcopy(d)
    del copied['authentication']['access_token']
    return copied
...
def RecipeRun(**kwargs):
    ...
    logger.info(f"parameters('access_token' removed!): {json.dumps(without_access_token(kwargs), indent=4)}")
    ...
    try:
        ...
        model = EfficientNet.from_name(hyperparameter['network_name'], num_classes=num_classes)
        model.to(device)
        
        criterion = torch.nn.CrossEntropyLoss(label_smoothing=hyperparameter['label_smoothing'])
        ...
        for epoch in range(1, hyperparameter['epoch']+1):
            ...
            if epoch % hyperparameter['save_epoch'] == 0 or epoch == hyperparameter['epoch']:
                ...
                mpp.daq.object_service.upload_model(model, uri=model_save_uri, inference_info=inference_info, example=inputs, channel=operation_channel, access_token=access_token, chunk_size=chunk_size)
...
def data_download(train_ratio,
    ...
    for i in range(len(classification_gts)):
        ...
        if image.ndim == 2: image = mpp.intel64.rgb(image)
