...
def RecipeRun(**kwargs):
    ...
    gt_data_set_id = kwargs['gt_dataset']['gt_dataset_id']
    segmentation_gts_list = get_segmentation_gts(operation_channel, access_token, gt_data_set_id)
    segmentation_gt_dataset = get_segmentation_gt_dataset(operation_channel, access_token, gt_data_set_id)
    label_info = sorted(load_label_info(operation_channel, access_token, segmentation_gt_dataset.class_code_set_id))

    logger.info("Temp Folder Create")

    main_dir = os.path.dirname(os.path.abspath(sys.modules['__main__'].__file__))
    local_download_path = os.path.join(main_dir, "temp", str(uuid.uuid4()))
    ...
    try:
        ...
        for epoch in range(1, hyperparameter['epoch']):
            ...
            for n_count, (images, labels) in enumerate(train_loader, 1):
                ...
                if n_count % max(total_iteration // 10, 1) == 0 or n_count == total_iteration:
