...
def get_efficient(name='B3', input_shape=(None,None,1)) :
...
def efficient_unet(input_shape = (224,224,3), num_classes=1) :
    encoder_model = get_efficient(name='B3', input_shape=input_shape)
...
def RecipeRun(**kwargs):
    ...
    src_path, gt_path = download_image(operation_channel,
                                        access_token,
    ...
    try:
        ...
        image_datagen = ImageDataGenerator(validation_split = 0.2, rescale = 1/255)
        mask_datagen = ImageDataGenerator(validation_split=0.2, rescale = 1/255)
        ...
        mask_generator = mask_datagen.flow_from_directory(gt_path,class_mode = None, seed=seed, target_size=(image_size,image_size),subset = 'training')
        val_mask_generator = mask_datagen.flow_from_directory(gt_path,class_mode = None, seed=seed, target_size=(image_size,image_size),subset = 'validation')
        ...
        model = efficient_unet(input_shape = (image_size, image_size, 3))
        model.compile(optimizer = 'adam', loss = 'binary_crossentropy', metrics = ['acc'])
        model.fit(train_generator, validation_data = validation_generator, epochs = int(hyperparameter['epoch']), validation_steps = validation_steps, steps_per_epoch = steps_per_epoch, callbacks=cb)


        model_save_uri = f"{result_uri}/model.h5"
        upload_model(model, uri=model_save_uri, channel=operation_channel, access_token=access_token, chunk_size=chunk_size)
...
def download_image(operation_channel, access_token, segmentation_gts, download_path):
