...
class ModelHandler:
    def __init__(self, data, context):
        ...
        self.model1 = torch.load(model_path, map_location=self.device)
        self.model1.to(self.device)
        self.model1.eval()
        logger.info(f"[LOG] model.pth 로드 성공")
        print_gpu_memory_info("model.pth 로딩 후")
        
        # 두 번째 모델 생성 (model.pth 구조 + weight2.pth 가중치)
        import copy
        self.model2 = copy.deepcopy(self.model1)  # 동일한 구조 복사
        
        weight2_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "weight2.pth")
        weight2_dict = torch.load(weight2_path, map_location=self.device)
        self.model2.load_state_dict(weight2_dict)
        self.model2.to(self.device)
        self.model2.eval()
        logger.info(f"[LOG] weight2.pth 가중치 로드 성공")
        print_gpu_memory_info("weight2.pth 로딩 후")
    ...
    def get_current_model(self):
        ...
        if selected_model_idx == 1:  # 홀수
            logger.info(f"[LOG] 소수점 이하: {decimal_part:.4f} -> {decimal_x100} % 2 = {selected_model_idx} -> model.pth 사용")
            return self.model1
        else:  # 짝수
            logger.info(f"[LOG] 소수점 이하: {decimal_part:.4f} -> {decimal_x100} % 2 = {selected_model_idx} -> weight2.pth 사용")
            return self.model2
