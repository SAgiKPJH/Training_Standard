...
class ModelHandler:
    def __init__(self, data, context):
        ...
        self.weight2_path = os.path.join(self.base_path, "weight2.pth")
        
        # 현재 상태 추적
        self.current_weight_type = None  # 'model' 또는 'weight2'
    ...
    def load_weight_if_needed(self, target_weight_type):
        ...
        try:
            if target_weight_type == 'model':
                # model.pth 가중치 로드
                weight_dict = torch.load(self.model_weight_path, map_location=self.device, weights_only=True)
                if hasattr(weight_dict, 'state_dict'):
                    weight_dict = weight_dict.state_dict()
                self.current_model.load_state_dict(weight_dict)
                logger.info(f"[WEIGHT CHANGE] model.pth 가중치 로드 완료")
                
            else:  # weight2
                # weight2.pth 가중치 로드
                weight2_dict = torch.load(self.weight2_path, map_location=self.device)
                self.current_model.load_state_dict(weight2_dict)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        logger.info(f"[TEST RESULT] 결과: {result_dict['result_code']}, 점수: {result_dict['score']:.4f}, 가중치: {result_dict['current_weight']}")
