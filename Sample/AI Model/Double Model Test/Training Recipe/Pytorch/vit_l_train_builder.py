...
class Torch_Model_Builder:
    ...
    def init_model(self, num_classes, device):
        # ViT-L/16 모델 사용 (Vision Transformer Large with 16x16 patch size)
    ...
    def init_optimizer(self, optimizer_name, lr):
        if optimizer_name.lower() == "adam":
            ...
            # ViT에 추천되는 AdamW optimizer 추가
            self.__optimizer = optim.AdamW(self.__model.parameters(), lr=lr, weight_decay=0.01)
    ...
    def train_step(self, batch, device, using_amp):
        ...
        with torch.cuda.amp.autocast(enabled=using_amp):
            # ViT는 auxiliary output이 없으므로 단일 output만 반환
