...
class ModelHandler:
    ...
    def clear_model(self):
        ...
        if self.model is not None:
            logger.info(f"[MODEL CLEAR] 기존 ViT 모델 메모리에서 제거 (폴더 2)")
            # 모델을 CPU로 이동 후 삭제
            self.model.cpu()
            del self.model
            self.model = None
    ...
    def initialize_model(self):
        ...
        logger.info(f"[MODEL INIT] 폴더 2 - Vision Transformer 모델 초기화 시작")
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                logger.info(f"[MODEL INIT] 폴더 2 - 기존 ViT model.pth 로드")
            else:
                # 다운로드된 가중치가 있는지 확인
                ...
                if download_weight_path and os.path.exists(download_weight_path):
                    ...
                    logger.info(f"[MODEL INIT] 폴더 2 - ViT 다운로드 가중치 로드 완료")
                else:
                    # 가중치가 없으면 사전 훈련된 모델 생성
                    logger.info(f"[MODEL INIT] 폴더 2 - 사전 훈련된 ViT 모델 생성")
            ...
            logger.info(f"[MODEL INIT] 폴더 2 - Vision Transformer 모델 초기화 완료")
            print_gpu_memory_info("ViT 모델 초기화 후")
            
        except Exception as e:
            logger.error(f"[MODEL INIT ERROR] 폴더 2 - Vision Transformer 모델 초기화 실패: {e}")
            self.is_model_loaded = False
            raise
    ...
    def __del__(self):
        ...
        try:
            self.clear_model()
            logger.info("[CLEANUP] 폴더 2 - ViT ModelHandler 정리 완료")
        except Exception as e:
            logger.error(f"[CLEANUP ERROR] 폴더 2 - ViT 정리 중 오류: {e}")
