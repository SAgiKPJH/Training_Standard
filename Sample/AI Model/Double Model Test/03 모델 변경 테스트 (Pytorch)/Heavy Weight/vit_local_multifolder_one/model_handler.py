...
class ModelHandler:
    """
    시간 기반 동적 Model Handler (Dynamic Loading 방식) - Vision Transformer 버전
    
    폴더 구조:
    /vit_local_multifolder_one/
    ...
    def __init__(self, data: Any, context: Dict[str, str]):
        ...
        self.logger.info("🚀 Time-based Dynamic Vision Transformer ModelHandler - Ready for dynamic loading...")
    ...
    def _clear_current_handler(self):
        ...
        if self._current_handler is not None:
            self.logger.info(f"[HANDLER CLEAR] 기존 ViT 핸들러 ({self._current_folder}) 메모리에서 제거")
            ...
            try:
                ...
                self.logger.warning(f"[HANDLER CLEAR] ViT 핸들러 정리 중 오류: {e}")
            ...
            self.logger.info(f"[HANDLER CLEAR] ViT 핸들러 제거 완료")
    ...
    def _load_handler_if_needed(self, target_folder: str, data: Any, context: Dict[str, str]):
        ...
        if self._current_folder == target_folder and self._current_handler is not None:
            self.logger.info(f"[HANDLER SKIP] 동일한 ViT 폴더 ({target_folder}) - 로딩 건너뛰기")
            return self._current_handler
        
        # 폴더가 다르면 기존 핸들러 제거 후 새로 로드
        if self._current_folder != target_folder:
            if self._current_folder is not None:
                self.logger.info(f"[HANDLER CHANGE] ViT 핸들러 변경 시작: {self._current_folder} -> {target_folder}")
    ...
    def _load_model_handler_from_folder(self, folder_name: str, data: Any, context: Dict[str, str]) -> Any:
        """선택된 폴더에서 실제 ViT ModelHandler 동적 로드 (각 폴더의 working directory에서 초기화)"""
        self.logger.info(f"[HANDLER LOAD] 새로운 ViT 핸들러 로드 시작: 폴더 {folder_name}")
        ...
        try:
            ...
            if not os.path.exists(model_handler_path):
                raise FileNotFoundError(f"ViT Model handler not found: {model_handler_path}")
            
            self.logger.debug(f"📂 Loading ViT from: {model_handler_path}")
            
            # ⚡ 해당 폴더의 working directory에서 초기화 (중요!)
            with self._working_directory(model_folder_path):
                self.logger.debug(f"🔄 Working directory for ViT initialization: {os.getcwd()}")
                
                # 동적 모듈 로드
                module_name = f"vit_model_handler_{folder_name}_{int(time.time())}"
                ...
                if hasattr(module, 'ModelHandler'):
                    self.logger.debug(f"🏗️ Creating ViT ModelHandler instance in folder: {folder_name}")
                    ...
                    self.logger.info(f"✅ ViT ModelHandler from folder {folder_name} loaded in {load_duration:.4f}s")
                    return handler_instance
                else:
                    raise AttributeError(f"ViT ModelHandler class not found in {model_handler_path}")
                
        except Exception as e:
            self.logger.error(f"Failed to load vit model handler from folder {folder_name}: {e}")
            # 실패시 현재 핸들러 상태 초기화
            self._current_handler = None
            self._current_folder = None
            raise
    ...
    def _determine_model_folder(self) -> str:
        """
        현재 시간의 초 단위를 기반으로 사용할 ViT 모델 폴더 결정
        ...
        self.logger.info(f"[VIT FOLDER SELECT] 시간: {current_time:.3f}, 초: {seconds}, %2 = {folder_selector} -> 폴더 {selected_folder}")
        
        return selected_folder
    
    def __call__(self, data: Any, context: Dict[str, str]) -> Tuple[Any, Dict[str, str]]:
        """
        실제 ViT 추론 수행 - 시간에 따라 적절한 폴더 선택하고 동적으로 핸들러 로드
        """
        ...
        try:
            ...
            self.logger.info(f"🔍 Current ViT request requires model folder: {required_folder}")
            self.logger.info(f"📊 ViT folder selection stats: 1번={self._folder_select_stats['folder_1_count']}, "
            ...
            if actual_handler is None:
                raise RuntimeError(f"Failed to load ViT handler for folder {required_folder}")
            ...
            output_context["used_model_type"] = "vision_transformer"
            ...
            self.logger.info(f"✅ ViT inference completed using dynamic model folder: {required_folder}")
            self.logger.info(f"⏱️ ViT call performance: total={total_call_time:.4f}s, inference={inference_time:.4f}s, "
                           f"select={select_duration:.6f}s, load={load_duration:.4f}s (dynamic)")
            
            return result, output_context
            
        except Exception as e:
            self.logger.error(f"❌ ViT inference failed: {e}")
            # 에러 정보도 context에 포함
            error_context = dict(context)
            error_context["error"] = str(e)
            error_context["used_model_folder"] = required_folder if 'required_folder' in locals() else "unknown"
            error_context["used_model_type"] = "vision_transformer"
            raise
    
    def test(self):
        """테스트 함수 - ViT 폴더 선택 테스트"""
        self.logger.info("[VIT TEST] 시간 기반 폴더 선택 테스트 시작...")
        
        for i in range(10):
            selected_folder = self._determine_model_folder()
            self.logger.info(f"[VIT TEST] {i+1}번째 테스트 - 선택된 폴더: {selected_folder}")
            time.sleep(1)  # 1초 대기로 초 변경 확인
        
        self.logger.info("[VIT TEST] 폴더 선택 테스트 완료")
        self.logger.info(f"[VIT TEST STATS] 폴더 선택 통계: {self._folder_select_stats}")
    ...
    def reinitialize_current_model(self):
        """현재 로드된 ViT 핸들러의 모델을 재초기화"""
        if self._current_handler is not None and hasattr(self._current_handler, 'initialize_model'):
            self.logger.info(f"[VIT MODEL REINIT] 현재 ViT 핸들러 ({self._current_folder}) 모델 재초기화")
            self._current_handler.initialize_model()
        else:
            self.logger.warning("[VIT MODEL REINIT] 재초기화할 ViT 핸들러가 없거나 initialize_model 메서드가 없음")

    def __del__(self):
        """소멸자에서 현재 ViT 핸들러 정리"""
        try:
            self._clear_current_handler()
            self.logger.info("[CLEANUP] Dynamic MultiFolder ViT ModelHandler 정리 완료")
        except Exception as e:
            self.logger.error(f"[CLEANUP ERROR] ViT 정리 중 오류: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(5):
            logger.info(f"\n[VIT TEST RUN] {i+1}번째 추론 테스트 실행")
            result, context = handler(data=data, context={})
            logger.info(f"[VIT TEST RESULT] 사용된 폴더: {context['used_model_folder']}")
            logger.info(f"[VIT TEST PERFORMANCE] {context['performance']}")
            time.sleep(2)  # 2초 대기로 폴더 변경 확인
            
    except Exception as e:
        logger.error(f"[VIT TEST ERROR] 테스트 실행 중 오류: {e}")
