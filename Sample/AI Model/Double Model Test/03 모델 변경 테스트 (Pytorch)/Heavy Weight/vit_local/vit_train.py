...
if __name__ == "__main__":
    ...
    try:
        ...
        # 3. 모델 생성 및 학습
        logger.info("Setting up ViT-Huge model...")
        ...
        # 5. 학습 실행
        logger.info("Starting ViT-Huge training...")
        model_builder.train(
            dataset_builder.get_train_loader(),
            dataset_builder.get_validation_loader()
        )

        logger.info("ViT-Huge training completed successfully!")

    except Exception as e:
        logger.error(f"ViT-Huge training failed: {e}")
        raise
