...
class ModelHandler:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                logger.info(f"[MODEL INIT] 폴더 2 - 기본 Swin-Large 모델 생성")
            ...
            logger.error(f"[MODEL INIT] 폴더 2 - Swin Transformer 모델 초기화 실패: {e}")
            raise

    def test(self):        ## 이후에 추론 하는 모든 데이터에 대해서 빠르게 진행 될 수 있도록 하는 부분
        """모델 테스트 - float32와 mixed precision 모두 테스트"""
        logger.info("[SWIN TEST] 폴더 2 - Swin Transformer 모델 테스트 시작")
        ...
        logger.info("[SWIN TEST] 폴더 2 - Swin Transformer 모델 테스트 완료")
    ...
    def __call__(self, data, context):
        ...
        try:
            ...
            logger.info(f"[SWIN INFERENCE] 폴더 2 - 클래스: {result['label']}, 신뢰도: {confidence:.4f}, 시간: {inference_time:.4f}s")
            
            return result, context
            
        except Exception as e:
            logger.error(f"[SWIN INFERENCE] 폴더 2 - 추론 실패: {e}")
            raise
    ...
    def __del__(self):
        ...
        try:
            self.clear_model()
            logger.info("[CLEANUP] 폴더 2 - Swin Transformer ModelHandler 정리 완료")
        except Exception as e:
