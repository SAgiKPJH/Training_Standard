...
class ModelHandler:
    def __init__(self, data, context):
        ...
        # Swin Transformer용 Transform 설정 (384x384)
    ...
    def clear_model(self):
        ...
        if self.model is not None:
            logger.info(f"[MODEL CLEAR] 기존 Swin 모델 메모리에서 제거 (폴더 2)")
            # 모델을 CPU로 이동 후 삭제
            self.model.cpu()
            del self.model
            self.model = None
    ...
    def initialize_model(self):
        ...
        logger.info(f"[MODEL INIT] 폴더 2 - Swin Transformer 모델 초기화 시작")
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                logger.info(f"[MODEL INIT] 폴더 2 - 기존 Swin model.pth 로드")
            else:
                # 다운로드된 가중치가 있는지 확인
                download_weight_path = None
                for file in os.listdir(self.base_path):
                    if file.endswith('.pth') and 'swin' in file.lower():
                        download_weight_path = os.path.join(self.base_path, file)
                        break
                
                if download_weight_path and os.path.exists(download_weight_path):
                    logger.info(f"[MODEL INIT] 폴더 2 - 다운로드된 가중치 사용: {download_weight_path}")
                    # 다운로드된 가중치는 state_dict이므로 모델 구조를 먼저 생성
                    self.model = create_model('swin_large_patch4_window12_384_in22k', pretrained=False, num_classes=len(self.label_list))
                    state_dict = torch.load(download_weight_path, map_location=self.device, weights_only=False)
                    self.model.load_state_dict(state_dict, strict=False)
                    logger.info(f"[MODEL INIT] 폴더 2 - Swin 다운로드 가중치 로드 완료")
                else:
                    # 가중치가 없으면 사전 훈련된 모델 생성
                    logger.info(f"[MODEL INIT] 폴더 2 - 사전 훈련된 Swin 모델 생성")
                    self.model = create_model('swin_large_patch4_window12_384_in22k', 
                                            pretrained=True, 
            ...
            logger.info(f"[MODEL INIT] 폴더 2 - Swin Transformer 모델 초기화 완료")
            print_gpu_memory_info("Swin 모델 초기화 후")
            
        except Exception as e:
            logger.error(f"[MODEL INIT ERROR] 폴더 2 - Swin Transformer 모델 초기화 실패: {e}")
            self.is_model_loaded = False
            raise

    def test(self):        ## 이후에 추론 하는 모든 데이터에 대해서 빠르게 진행 될 수 있도록 하는 부분
        print_gpu_memory_info("Swin 모델 테스트 전")
        
        # 더미 데이터로 워밍업 (Swin: 384x384)
        ...
        print_gpu_memory_info("Swin 모델 테스트 완료 후")

    def __call__(self, data, context):
        print_gpu_memory_info("Swin 모델 추론 전")
        ...
        # Resize images to match Swin's expected input size (384x384)
        ...
        try:
            ...
            logger.info(f"[SWIN PREDICT] 추론 성공 (float32)")
        except Exception as e:
            logger.warning(f"[SWIN PREDICT] float32 추론 실패: {e}, mixed precision으로 재시도")
            try:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=True):
                        predict_result = self.model(images)
                        predict_result = predict_result.cpu().numpy()
                logger.info(f"[SWIN PREDICT] 추론 성공 (mixed precision)")
            except Exception as e2:
                logger.error(f"[SWIN PREDICT] mixed precision 추론도 실패: {e2}")
                raise e2
        
        predict_end_time = time.time()
        predict_duration = predict_end_time - predict_start_time
        logger.info(f"[SWIN PREDICT TIME] 추론 시간: {predict_duration:.4f}초")
        
        print_gpu_memory_info("Swin 모델 추론 후")
        ...
        result['model_type'] = 'swin_transformer'
        result['folder'] = '2'
        
        result = pickle.dumps(result)
        
        print_gpu_memory_info("Swin 완료 후")
    ...
    def __del__(self):
        ...
        try:
            self.clear_model()
            logger.info("[CLEANUP] 폴더 2 - Swin ModelHandler 정리 완료")
        except Exception as e:
            logger.error(f"[CLEANUP ERROR] 폴더 2 - Swin 정리 중 오류: {e}")
