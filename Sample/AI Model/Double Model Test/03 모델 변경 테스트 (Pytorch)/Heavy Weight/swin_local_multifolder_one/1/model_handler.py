...
class ModelHandler:
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                if download_weight_path and os.path.exists(download_weight_path):
                    ...
                    logger.info(f"[MODEL INIT] 폴더 1 - Swin 다운로드 가중치 로드 완료")
                else:
                    # 가중치가 없으면 사전 훈련된 모델 생성
                    logger.info(f"[MODEL INIT] 폴더 1 - 사전 훈련된 Swin 모델 생성")
            ...
            logger.error(f"[MODEL INIT ERROR] 폴더 1 - Swin Transformer 모델 초기화 실패: {e}")
            self.is_model_loaded = False
            raise
    ...
    def __del__(self):
        ...
        try:
            self.clear_model()
            logger.info("[CLEANUP] 폴더 1 - Swin ModelHandler 정리 완료")
        except Exception as e:
