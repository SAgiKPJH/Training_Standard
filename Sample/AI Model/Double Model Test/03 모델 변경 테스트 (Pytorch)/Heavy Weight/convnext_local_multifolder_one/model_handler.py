...
class ModelHandler:
    """
    시간 기반 동적 Model Handler (Dynamic Loading 방식)
    
    폴더 구조:
    /convnext_local_multifolder/
    ...
    def __init__(self, data: Any, context: Dict[str, str]):
        ...
        self.logger.info("🚀 Time-based Dynamic ModelHandler - Ready for dynamic loading...")
    ...
    def _clear_current_handler(self):
        ...
        if self._current_handler is not None:
            self.logger.info(f"[HANDLER CLEAR] 기존 핸들러 ({self._current_folder}) 메모리에서 제거")
            ...
            try:
                ...
                self.logger.warning(f"[HANDLER CLEAR] 핸들러 정리 중 오류: {e}")
            ...
            self.logger.info(f"[HANDLER CLEAR] 핸들러 제거 완료")
    ...
    def _load_handler_if_needed(self, target_folder: str, data: Any, context: Dict[str, str]):
        ...
        if self._current_folder == target_folder and self._current_handler is not None:
            self.logger.info(f"[HANDLER SKIP] 동일한 폴더 ({target_folder}) - 로딩 건너뛰기")
            return self._current_handler
        
        # 폴더가 다르면 기존 핸들러 제거 후 새로 로드
        if self._current_folder != target_folder:
            if self._current_folder is not None:
                self.logger.info(f"[HANDLER CHANGE] 핸들러 변경 시작: {self._current_folder} -> {target_folder}")
    ...
    def _load_model_handler_from_folder(self, folder_name: str, data: Any, context: Dict[str, str]) -> Any:
        ...
        self.logger.info(f"[HANDLER LOAD] 새로운 핸들러 로드 시작: 폴더 {folder_name}")
        ...
        try:
            ...
            with self._working_directory(model_folder_path):
                ...
                if hasattr(module, 'ModelHandler'):
                    ...
                    self.logger.info(f"✅ ModelHandler from folder {folder_name} loaded in {load_duration:.4f}s")
    ...
    def _determine_model_folder(self) -> str:
        """
        현재 시간의 초 단위를 기반으로 사용할 모델 폴더 결정
        ...
        self.logger.info(f"[FOLDER SELECT] 시간: {current_time:.3f}, 초: {seconds}, %2 = {folder_selector} -> 폴더 {selected_folder}")
        
        return selected_folder
    
    def __call__(self, data: Any, context: Dict[str, str]) -> Tuple[Any, Dict[str, str]]:
        """
        실제 추론 수행 - 시간에 따라 적절한 폴더 선택하고 동적으로 핸들러 로드
        """
        ...
        try:
            ...
            self.logger.info(f"📊 Folder selection stats: 1번={self._folder_select_stats['folder_1_count']}, "
            ...
            if actual_handler is None:
                raise RuntimeError(f"Failed to load handler for folder {required_folder}")
            ...
            self.logger.info(f"✅ Inference completed using dynamic model folder: {required_folder}")
            self.logger.info(f"⏱️ Call performance: total={total_call_time:.4f}s, inference={inference_time:.4f}s, "
    ...
    def test(self):
        """테스트 함수 - 폴더 선택 테스트"""
        self.logger.info("[TEST] 시간 기반 폴더 선택 테스트 시작...")
        
        for i in range(10):
            selected_folder = self._determine_model_folder()
            self.logger.info(f"[TEST] {i+1}번째 테스트 - 선택된 폴더: {selected_folder}")
            time.sleep(1)  # 1초 대기로 초 변경 확인
        
        self.logger.info("[TEST] 폴더 선택 테스트 완료")
        self.logger.info(f"[TEST STATS] 폴더 선택 통계: {self._folder_select_stats}")
    ...
    def reinitialize_current_model(self):
        """현재 로드된 핸들러의 모델을 재초기화"""
        if self._current_handler is not None and hasattr(self._current_handler, 'initialize_model'):
            self.logger.info(f"[MODEL REINIT] 현재 핸들러 ({self._current_folder}) 모델 재초기화")
            self._current_handler.initialize_model()
        else:
            self.logger.warning("[MODEL REINIT] 재초기화할 핸들러가 없거나 initialize_model 메서드가 없음")

    def __del__(self):
        """소멸자에서 현재 핸들러 정리"""
        try:
            self._clear_current_handler()
            self.logger.info("[CLEANUP] Dynamic MultiFolder ModelHandler 정리 완료")
        except Exception as e:
            self.logger.error(f"[CLEANUP ERROR] 정리 중 오류: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(5):
            logger.info(f"\n[TEST RUN] {i+1}번째 추론 테스트 실행")
            result, context = handler(data=data, context={})
            logger.info(f"[TEST RESULT] 사용된 폴더: {context['used_model_folder']}")
            logger.info(f"[TEST PERFORMANCE] {context['performance']}")
            time.sleep(2)  # 2초 대기로 폴더 변경 확인
            
    except Exception as e:
        logger.error(f"[TEST ERROR] 테스트 실행 중 오류: {e}")
