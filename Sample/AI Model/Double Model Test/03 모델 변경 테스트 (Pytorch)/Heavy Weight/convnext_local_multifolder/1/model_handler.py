...
class ModelHandler:
    ...
    def clear_model(self):
        ...
        if self.model is not None:
            logger.info(f"[MODEL CLEAR] 기존 모델 메모리에서 제거 (폴더 1)")
            # 모델을 CPU로 이동 후 삭제
            self.model.cpu()
            del self.model
            self.model = None
    ...
    def initialize_model(self):
        """모델을 새로 초기화 (기존 모델 제거 후)"""
        logger.info(f"[MODEL INIT] 폴더 1 - ConvNeXt 모델 초기화 시작")
        ...
        try:
            ...
            base_model_path = os.path.join(f"{model_name}_{num_classes}_base.model")
            ...
            if os.path.exists(base_model_path):
                ...
                raise FileNotFoundError(f"Please run save_base_model.py first to create {base_model_path}")
            ...
            logger.info(f"[MODEL INIT] 폴더 1 - ConvNeXt 모델 초기화 완료")
            print_gpu_memory_info("모델 초기화 후")
            
        except Exception as e:
            logger.error(f"[MODEL INIT ERROR] 폴더 1 - ConvNeXt 모델 초기화 실패: {e}")
            self.is_model_loaded = False
            raise
    ...
    def __del__(self):
        ...
        try:
            self.clear_model()
            logger.info("[CLEANUP] 폴더 1 - ModelHandler 정리 완료")
        except Exception as e:
            logger.error(f"[CLEANUP ERROR] 폴더 1 - 정리 중 오류: {e}")
