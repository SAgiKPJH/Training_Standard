...
class ModelHandler:
    def __init__(self, data, context):
        ...
        # 11개의 모델을 모두 로드 (model0.h5 ~ model10.h5)
        self.models = {}
        base_path = os.path.dirname(os.path.abspath(__file__))
        
        print(f"[LOG] 11개 모델 로드 시작...")
    ...
    def get_current_model(self):
        """현재 시간의 소수점 이하 부분을 *100해서 %11로 모델 선택 (0~10)"""
        current_time = time.time()
        decimal_part = current_time - int(current_time)  # 소수점 이하 부분
        decimal_x100 = int(decimal_part * 100)
        selected_model_idx = decimal_x100 % 11
        
        print(f"[LOG] 소수점 이하: {decimal_part:.4f} -> {decimal_x100} % 11 = {selected_model_idx} -> model{selected_model_idx}.h5 사용")
