...
class ModelHandler:
    def __init__(self, data, context):
        ...
        # 100개의 모델을 모두 로드 (model0.h5 ~ model99.h5)
        self.models = {}
        base_path = os.path.dirname(os.path.abspath(__file__))
        
        print(f"[LOG] 100개 모델 로드 시작...")
        for i in range(100):
            ...
            if os.path.exists(model_path):
                try:
                    ...
                    if i < 10 or i % 10 == 0:  # 처음 10개와 10의 배수만 로그 출력
    ...
    def test(self) :        ## 이후에 추론 하는 모든 데이터에 대해서 빠르게 진행 될 수 있도록 하는 부분
        # 모든 모델에 대해 test 실행 (처음 10개만 로그 출력)
        print(f"[LOG] {len(self.models)}개 모델 테스트 시작...")
        ...
        for i, model in self.models.items():
            try:
                ...
                if i < 10:  # 처음 10개만 로그 출력
                    ...
                    print(f"[LOG] ... (로그 생략, 총 {len(self.models)}개 모델 테스트 중)")
                    
            except Exception as e:
                print(f"[ERROR] model{i}.h5 test 실패: {e}")
        
        print(f"[LOG] 모델 테스트 완료")

    def get_current_model(self):
        """현재 시간의 소수점 이하 부분을 *100해서 %100으로 모델 선택 (0~99)"""
        current_time = time.time()
        decimal_part = current_time - int(current_time)  # 소수점 이하 부분
        decimal_x100 = int(decimal_part * 100)
        selected_model_idx = decimal_x100 % 100
        
        print(f"[LOG] 소수점 이하: {decimal_part:.4f} -> {decimal_x100} % 100 = {selected_model_idx} -> model{selected_model_idx}.h5 사용")
