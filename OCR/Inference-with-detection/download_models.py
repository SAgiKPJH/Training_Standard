...
다운로드 대상:
  ...
  - en_dict.txt (표준 문자 사전)                              -> data/en_dict_pretrained.txt

* 커스텀 학습 rec 모델은 data/rec_model/ 에 직접 배치하세요.
* pretrained rec 모델 사용 시 model.json 에서:
    rec_model_dir        = "data/rec_model_pretrained"
    rec_char_dict_path   = "data/en_dict_pretrained.txt"
"""
...
def _copy_pretrained_dict():
    ...
    try:
        ...
        print(f"  Copied -> data/en_dict_pretrained.txt")
    except ImportError:
        print("[warn] paddleocr 미설치 — en_dict_pretrained.txt 복사를 건너뜁니다.")
