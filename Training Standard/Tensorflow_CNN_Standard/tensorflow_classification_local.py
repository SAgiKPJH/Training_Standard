...
from Builder import Local_Tensorflow_ClassificationDatasetBuilder
dataset_builder = Local_Tensorflow_ClassificationDatasetBuilder(logger=logger).init_dataset_path(dataset_path=dataset
                  ).init_transform(
    ...
    def on_epoch_end(self, total_epoch, epoch, train_loss, validation_loss, epoch_elapsed_time, model):
        ...
        if self.__best_loss is None or loss < self.__best_loss:
            ...
            if self.__daq_old_path:
                ...
                self.__save_builder.save_model(f"best_loss/model", model=model)
        ...
        if self.__best_acc is None or acc > self.__best_acc:
            ...
            if self.__daq_old_path:
                ...
                self.__save_builder.save_model(f"best_acc/model", model=model)
